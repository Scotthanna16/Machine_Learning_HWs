...
def train_one_vs_all(X, y, num_classes, lambda_val):
    ...
    for i in range(num_classes):
        ...
        for j in range(m):
            if(i==y[j]):
                temp[j]=1
        weight_vector, intercept = train_logistic_regression(X, temp, lambda_val)
        weight_vectors[:,[i]]=np.transpose(np.array([weight_vector]))
...
def predict_one_vs_all(X, weight_vectors, intercepts):
    ...
    m,n = X.shape
    q,num= weight_vectors.shape
    predictions=[0]*m
  
    for i in range(m):
        temp=[0]*num
        for j in range(num):
            temp[j]=np.vdot(np.transpose(weight_vectors[:,[j]]),X[i])+intercepts[j]
        predictions[i]=np.argmax(temp)
